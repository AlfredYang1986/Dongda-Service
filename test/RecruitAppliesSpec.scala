...
//    pushApplyTest               $pushApplyTest
    ...
    def pushApplyTest = {
        WsTestClient.withClient { client =>
            val reVal = Await.result(
                new DongdaClient(client, "http://127.0.0.1:9000").pushRecruitApply(token, user_id, apply_info), time_out)
    ...
    def popApplyTest = {
        WsTestClient.withClient { client =>
            val reVal = Await.result(
                new DongdaClient(client, "http://127.0.0.1:9000").popRecruitApply(token, apply_id), time_out)
            (reVal \ "status").asOpt[String].get must_== "ok"
    ...
    def queryApplyTest = {
        WsTestClient.withClient { client =>
            val reVal = Await.result(
                new DongdaClient(client, "http://127.0.0.1:9000").queryRecruitApply(token, apply_id), time_out)
            (reVal \ "status").asOpt[String].get must_== "ok"
    ...
    def updateApplyTest = {
        WsTestClient.withClient { client =>
            val reVal = Await.result(
                new DongdaClient(client, "http://127.0.0.1:9000").updateRecruitApply(token, apply_id, apply_update_info), time_out)
            (reVal \ "status").asOpt[String].get must_== "ok"
