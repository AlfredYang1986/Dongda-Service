...
//            val trade_no = (data \ "order_id").asOpt[String].map (x => x).getOrElse(throw new Exception("wrong input"))
           ...
           val fee = (data \ "total_fee").asOpt[Int].map (x => x).getOrElse(throw new Exception("input error"))
            val spbill_create_ip = (data \ "spbill_create_ip").asOpt[String].map (x => x).getOrElse(WechatSercurityFunc.spbill_create_ip)
        		...
        		val str_md5 = bmlogic.common.sercurity.Sercurity.md5Hash(str_pay).toUpperCase
        		val valxml = """<xml>
        		                  <appid>%s</appid>
        		                  <body><![CDATA[%s]]></body>
        		                  <mch_id>%s</mch_id>
