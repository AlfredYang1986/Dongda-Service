    ...
    implicit val rc : JsValue => DBObject = { js =>
        ...
        def payment_time : Option[DBObject] = {
            (data \ "payment_time").asOpt[JsValue].map { tmp =>
                val builder = MongoDBObject.newBuilder
                builder += "price" -> (tmp \ "price").asOpt[Double].get
        ...
        def payment_monthly : Option[DBObject] = {
            (data \ "payment_monthly").asOpt[JsValue].map { tmp =>
                val builder = MongoDBObject.newBuilder
                builder += "full_time" -> (tmp \ "full_time").asOpt[Double].get
                builder += "half_time" -> (tmp \ "half_time").asOpt[Double].get
                Some(builder.result)
        ...
        val builder = MongoDBObject.newBuilder
        builder += "recruit_id" -> new ObjectId
